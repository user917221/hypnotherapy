...
export default function CarteCadeauClient() {
    ...
    const selectedOption = useCustomAmount
        ? { title: "Montant Libre", price: `${customAmount || "0"} €`, priceNum: Number(customAmount) || 0, service: "Hypnose ou Sophrologie", duration: "1h30" }
        : giftOptions[selected];

    const currentPrice = useCustomAmount ? (Number(customAmount) || 0) : selectedOption.priceNum;
    ...
    const handleApplePay = () => {
        ...
        alert(`Paiement Apple Pay de ${currentPrice} € pour "${selectedOption.title}"\nCarte envoyée à : ${email}`);
    };
                    ...
                    <div className="space-y-4">
                        ...
                        {giftOptions.map((option, i) => {
                            ...
                            return (
                                <motion.button
                                    key={i}
                                    onClick={() => { setSelected(i); setUseCustomAmount(false); }}
                        ...
                        <motion.button
                            ...
                            className={`relative w-full text-left p-6 rounded-[2rem] overflow-hidden flex items-start gap-5 border transition-all duration-400 ${useCustomAmount
                                ? "bg-[var(--theme-accent)]/10 border-[var(--theme-accent)] shadow-xl shadow-[var(--theme-accent)]/15"
                                : "border-[var(--theme-text)]/10 hover:border-[var(--theme-accent)]/30 bg-[var(--theme-text)]/4"
                                }`}
                        >
                            <div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 transition-all duration-400 ${useCustomAmount ? "bg-[var(--theme-accent)] text-[var(--theme-bg)]" : "bg-[var(--theme-text)]/8 text-[var(--theme-accent)]"}`}>
                                <Sparkles className="w-6 h-6" />
                            </div>
                            <div className="flex-1 min-w-0 pr-8">
                                <p className={`text-[10px] uppercase tracking-[0.2em] font-bold font-sans mb-1 transition-colors ${useCustomAmount ? "text-[var(--theme-accent)]" : "text-[var(--theme-text)]/40"}`}>
                                    Libre · 1h30
                                </p>
                                <h2 className="font-serif-display text-xl text-[var(--theme-text)] mb-1">Montant Personnalisé</h2>
                                <p className="font-sans text-xs font-light text-[var(--theme-text)]/55 leading-relaxed">Définissez vous-même la valeur du cadeau.</p>
                            </div>
                            {useCustomAmount && (
                                <motion.div
                                    initial={{ scale: 0 }}
                                    animate={{ scale: 1 }}
                                    className="w-6 h-6 rounded-full bg-[var(--theme-accent)] flex items-center justify-center pt-1"
                                >
                                    <Check className="w-3.5 h-3.5 text-[var(--theme-bg)] stroke-[3]" />
                                </motion.div>
                            )}
                    ...
                    >
                        ...
                        <div>
                            ...
                            <AnimatePresence mode="wait">
                                <motion.div
                                    key={useCustomAmount ? "custom" : selected}
                        ...
                        {useCustomAmount && (
                            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
                                <label className="text-[10px] uppercase tracking-[0.25em] font-bold text-[var(--theme-text)]/40 font-sans block ml-1">
                                    Saisir le montant (€)
                                </label>
                                <input
                                    type="number"
                                    ...
                                    className="w-full px-5 py-4 rounded-2xl bg-[var(--theme-text)]/5 border border-[var(--theme-accent)]/30 text-[var(--theme-text)] placeholder:text-[var(--theme-text)]/25 font-sans text-xl font-bold focus:outline-none focus:border-[var(--theme-accent)]/50 focus:bg-[var(--theme-accent)]/5 transition-all"
                                />
                            </motion.div>
                        )}
